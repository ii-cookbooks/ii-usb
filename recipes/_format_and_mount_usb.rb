...
if node['ii-usb']['partition-size'] != '-1'
  # we would need more logic to see if there is any room remaining
  bash "partition and format #{node['ii-usb']['target-device']}" do
    # don't format if we already have the correct volume name
    not_if "blkid -s LABEL #{node['ii-usb']['target-device']}2 | grep casper-rw"
    code <<-eoc
